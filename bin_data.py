...
def process_data(input_file):
    ...
    pollutant_labels = ['低', '中低', '中高', '高']
    for col in pollutants:
        bins = np.linspace(df[col].min(), df[col].max(), num=len(pollutant_labels) + 1)
        ...
        for i in range(len(pollutant_labels)):
    ...
    bins_ws = np.linspace(df[ws_col].min(), df[ws_col].max(), num=len(windspeed_labels) + 1)
    ...
    for i in range(len(windspeed_labels)):
        erange_records.append({
            'variable': ws_col,
...
process_data("air_quality_data\\20250401_20250531_cleaned.csv")